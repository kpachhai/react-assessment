...
describe('MonstersList', () => {
  ...
  it('should dispatch the selected monster to the store', () => {
    monstersListFactory();
    act(() => screen.getByTestId('monster-1').click());
    const selectedMonster = store.getState().monsters.selectedMonster;
    expect(selectedMonster).toEqual(monstersData.monsters[0]);
  });

  it('should update the store to null when unselecting a monster', () => {
    monstersListFactory();
    act(() => screen.getByTestId('monster-1').click()); // Select
    act(() => screen.getByTestId('monster-1').click()); // Unselect
    const selectedMonster = store.getState().monsters.selectedMonster;
    expect(selectedMonster).toBeNull();
  });
});
